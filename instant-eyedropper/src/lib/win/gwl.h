...
template<typename type> inline type getwindownlong(HWND hwnd, int which = GWLP_USERDATA)
{
    return reinterpret_cast<type>(GetWindowLongPtr(hwnd, which));
}

template<typename type> inline void setwindownlong(HWND hwnd, type value, int which = GWLP_USERDATA)
{
