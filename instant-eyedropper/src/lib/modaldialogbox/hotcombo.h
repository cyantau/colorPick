...
public:
    hotcombo(HWND hwndparent, int id) : simplecontrol (hwndparent, id)
    {
        ...
        {
            // сам комбобокс
            WNDPROC phcorigproc = (WNDPROC)(DWORD_PTR)SetWindowLong(hcomb, GWLP_WNDPROC, (LONG)(DWORD_PTR)hotcomboproc);
            SendMessage(hcomb, MSG_HC_CREATE, (WPARAM)phcorigproc, (LPARAM)_hotcombodata);

            // эдитконтрол в комбобоксе
            WNDPROC pceorigproc = (WNDPROC)(DWORD_PTR)SetWindowLong(hedit, GWLP_WNDPROC, (LONG)(DWORD_PTR)combeditproc);
            SendMessage(hedit, MSG_CE_CREATE, (WPARAM)pceorigproc, (LPARAM)_hotcombodata);

            // лист в комбобоксе
            WNDPROC pclorigproc = (WNDPROC)(DWORD_PTR)SetWindowLong(hlist, GWLP_WNDPROC, (LONG)(DWORD_PTR)comblistproc);
